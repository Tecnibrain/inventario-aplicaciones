...
function vAplicaciones(A, rows) {
  ...
  // y las criticas, que en un parque real son un puñado: el resto de la tabla
  // sale con un guion y nada explicaba por que.
  const puntuan = data.filter(x => x.estado !== 'na').length;
  const aviso = puntuan < data.length ? `<div class="banner" style="margin:14px 0 0">${ico('info')}<div>
      <b>${fmt(puntuan)} de ${fmt(data.length)} aplicaciones puntúan en el cumplimiento.</b>
      El resto sale con «—» en <i>% al día</i>, que no es cero: es <b>no puntúa</b>.
      ${CFG.params.alcance === 'todas'
        ? 'Están fuera porque todavía no tienen versión aprobada.'
        : 'El alcance del estándar son las aplicaciones <b>administradas</b>, las <b>críticas</b> y las ' +
          '<b>no permitidas</b>: un equipo lleva decenas de librerías y controladores, y exigir que todos ' +
          'estén en la última versión convierte el indicador en ruido. ' +
          'Para puntuarlas todas, cambia el alcance a «todo el catálogo» en Administración.'}
    </div></div>` : '';

  return viewHead('Aplicaciones', 'Catálogo detectado en el parque. Pulsa una aplicación para ver su distribución de versiones y los equipos afectados.') + aviso +
