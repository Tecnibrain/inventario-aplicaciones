  ...
  return viewHead('Origen de datos',
    ...
    `<div class="card">
      <div class="row" style="margin-bottom:12px">
        ...
        <button class="btn" data-kqlact="graph" title="Copia el cuerpo JSON listo para pegar en Graph Explorer">Cuerpo JSON</button>
      ...
      <div class="fld" style="margin-top:12px">
        <label for="kqlFiltro">Filtro de equipos · una condición por línea (se inserta en las tres consultas)</label>
        <textarea id="kqlFiltro" data-cfg="kql.filtro" spellcheck="false" rows="3"
          placeholder='| where DeviceType == "Workstation"&#10;| where DomainName == "BANCOLOMBIA"'
          style="width:100%;background:var(--surface-2);border:1px solid var(--line);border-radius:8px;
                 padding:9px 11px;font-family:ui-monospace,Consolas,monospace;font-size:12px;
                 color:var(--ink);line-height:1.6;resize:vertical">${esc(kqlFiltro())}</textarea>
        <span class="hint">Escribe tantas condiciones como necesites, cada una en su línea. Por ejemplo:
          <code>| where DeviceType == "Workstation"</code>,
          <code>| where DomainName == "BANCOLOMBIA"</code>,
          <code>| where MachineGroup == "..."</code>,
          <code>| where DeviceName startswith "pb0b"</code>.
    ...
    `<div class="adm-grid">
      <div class="card"><div class="card-h"><div><h3>Ejecutar en Graph Explorer</h3>
        <p>Usa la aplicación de Microsoft, no la tuya</p></div></div>
        <div style="margin-top:14px;font-size:12.5px;color:var(--ink-3);line-height:1.7">
          <ol style="margin:0;padding-left:20px">
            <li>Abre <a href="https://developer.microsoft.com/graph/graph-explorer" target="_blank"
                rel="noopener" style="color:var(--brand)">Graph Explorer</a> e inicia sesión con tu cuenta.</li>
            <li>Método <b>POST</b>, versión <b>v1.0</b>, y esta dirección:
              <div class="mono mini" style="background:var(--surface-2);border:1px solid var(--line);
                   border-radius:7px;padding:7px 10px;margin:6px 0;word-break:break-all">https://graph.microsoft.com/v1.0/security/runHuntingQuery</div></li>
            <li>En la pestaña <b>Request body</b> pega el cuerpo: botón <b>Cuerpo JSON</b> arriba, en Consultas.</li>
            <li>En <b>Modify permissions</b> concede <code>ThreatHunting.Read.All</code>.</li>
            <li><b>Run query</b>, copia la respuesta y pégala aquí abajo.</li>
          </ol>
        </div>
      </div>
      <div class="card"><div class="card-h"><div><h3>Pegar la respuesta</h3>
        <p>El JSON que devuelve Graph Explorer entra directo al modelo</p></div></div>
        <div style="margin-top:14px">
          <textarea id="gxPegar" spellcheck="false" placeholder='{ "results": [ { "DeviceName": "...", ... } ] }'
            style="width:100%;min-height:150px;background:var(--surface-2);border:1px solid var(--line);
                   border-radius:9px;padding:11px;font-family:ui-monospace,Consolas,monospace;font-size:11.5px;
                   color:var(--ink);line-height:1.5;resize:vertical"></textarea>
          <div class="row" style="margin-top:10px">
            <select class="fsel" id="gxNombre" style="max-width:none">
              <option value="parque">Es el parque de equipos</option>
              <option value="catalogo" selected>Es el catálogo agregado</option>
              <option value="excepciones">Son las excepciones</option>
              <option value="otro">Otro</option>
            </select>
            <label class="chk"><input type="checkbox" id="gxReset"> Reemplazar lo cargado</label>
            <button class="btn btn-p" data-gx="pegar" style="margin-left:auto">Cargar respuesta</button>
          </div>
          <p class="mini" style="margin-top:10px">La forma se detecta sola por las columnas; el nombre es solo
            para reconocerlo en la lista de fuentes. Si la respuesta es muy larga, Graph Explorer puede tardar
            en mostrarla: usa <b>Contar filas</b> antes para saber a qué te enfrentas.</p>
        </div>
      </div>
    </div>` +
    ...
    `<div class="adm-grid">
      <div class="card"><div class="card-h"><div><h3>Script de PowerShell</h3>
        ...
        <div style="margin-top:14px;font-size:12.5px;color:var(--ink-3);line-height:1.65">
          ...
          <div class="fld"><label for="psModo">Cómo se autentica</label>
            <select id="psModo">
            ...
            <span class="hint" id="psAyuda">${AYUDA_MODO.aplicacion}</span>
          </div>
    ...
    sec('HP Workforce Experience', 'Tu cuenta, sin secreto y sin tope de filas: es el camino con menos trámite de los tres') +
    `<div class="adm-grid">
      <div class="card"><div class="card-h"><div><h3>Script de extracción</h3>
        <p>Inicias sesión una vez; a partir de ahí va solo</p></div></div>
        <div style="margin-top:14px">
          <div class="fld"><label for="hpMail">Tu correo corporativo</label>
            <input id="hpMail" data-cfg="hp.correo" value="${esc((CFG.hp || {}).correo || '')}"
                   placeholder="nombre@empresa.com" spellcheck="false">
            <span class="hint">El que usas en Workforce Experience, no el personal.</span></div>
          <div class="fld"><label for="hpRegion">Región de tu tenant</label>
            <select id="hpRegion" data-cfg="hp.region">
              <option value="us"${((CFG.hp || {}).region || 'us') === 'us' ? ' selected' : ''}>Estados Unidos</option>
              <option value="eu"${(CFG.hp || {}).region === 'eu' ? ' selected' : ''}>Europa</option>
              <option value="au"${(CFG.hp || {}).region === 'au' ? ' selected' : ''}>Australia</option>
            </select></div>
          <button class="btn btn-p" data-gx="hp">Descargar extraer-hp.ps1</button>
          <p class="mini" style="margin-top:12px">Al ejecutarlo te abre el navegador, inicias sesión y pegas
            la URL a la que te lleva. Eso es <b>todo</b>, y solo la primera vez: la sesión queda guardada
            cifrada en tu equipo y las siguientes ejecuciones no preguntan nada.</p>
        </div>
      </div>
      <div class="card"><div class="card-h"><div><h3>Por qué esto es un script y no un botón</h3>
        <p>Lo comprobé antes de escribirlo</p></div></div>
        <div style="margin-top:14px;font-size:12.5px;color:var(--ink-3);line-height:1.7">
          <p style="margin:0 0 10px">La pasarela de HP filtra por cabecera <code>Origin</code>. La misma
            llamada responde <b>200</b> desde el portal de HP y <b>403</b> desde cualquier otro sitio.
            Un navegador siempre manda <code>Origin</code>; un script no, y por eso sí pasa.</p>
          <p style="margin:0 0 10px">No es una limitación del tablero: <b>ninguna</b> página web podría
            hablar con esa API.</p>
          <div class="banner" style="margin:0">${ico('shield')}<div>
            El <i>refresh token</i> se guarda cifrado con DPAPI, junto al script: solo lo puede leer tu
            usuario en tu equipo. No pasa por esta página ni por el repositorio.
          </div></div>
        </div>
      </div>
      <div class="card"><div class="card-h"><div><h3>Qué trae, y qué hay que mirar</h3>
        <p>El script te lista tus tablas al arrancar</p></div></div>
        <div style="margin-top:14px;font-size:12.5px;color:var(--ink-3);line-height:1.7">
          <p style="margin:0 0 10px"><b>Sin tope de filas.</b> El lenguaje de consulta pagina con
            <code>limit</code> y <code>skip</code>, así que el script recorre todo. Es lo que resuelve el
            corte de 135.000 filas del export de la consola, que solo cubría 1.332 de 25.875 equipos.</p>
          <p style="margin:0 0 10px">Saca <b>catálogo agregado con desglose por versión</b> —lo que al
            Excel le faltaba—, <b>detalle por equipo</b> y <b>parque</b>.</p>
          <p style="margin:0">Lo que hay que comprobar: si entre tus tablas aparece
            <code>SoftwareInventory</code>, tienes lo instalado. Si solo están las de
            <code>...Utilization</code>, eso mide lo que se ha <b>usado</b>, que no es lo mismo.
            El script lo detecta, escoge la mejor y te avisa.</p>
        </div>
      </div>
    </div>` +
