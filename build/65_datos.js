...
# El permiso ThreatHunting.Read.All esta concedido como permiso de APLICACION,
...
# variable de entorno DEFENDER_SECRET si prefieres automatizarlo sin teclearlo.
$Client = '${kqlEsc(g.clientId || '')}'
$Tenant = '${kqlEsc(g.tenantId || '')}'
if (-not $Client -or -not $Tenant) { throw 'Faltan el Application ID y el Directory ID. Rellenalos en Origen de datos y vuelve a descargar el script.' }
...
if ($env:DEFENDER_SECRET) {
    $plano = $env:DEFENDER_SECRET
    Write-Host 'Usando el secreto de la variable de entorno DEFENDER_SECRET.' -ForegroundColor DarkGray
} else {
...
${modo === 'aplicacion' ? authApp : modo === 'dispositivo' ? authRest : authModulo}${plantilla}${cuerpo}
Write-Host ''
